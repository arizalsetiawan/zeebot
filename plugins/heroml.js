...
let handler = async(m, { conn, text }) => {
  ...
  await m.reply('Searching...')
    let dann = await fetch(`https://api.betabotz.org/api/search/heroml?hero=${text}&apikey=Izumi79109`)
    let res = await dann.json()
    let thumb = `https://api.lolhuman.xyz/api/gimage?apikey=Izumi123456&query=hero%20${text}`
    let hasil = `*── 「 HERO ML 」 ──*
